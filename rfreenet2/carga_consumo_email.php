...
while($cadena_actual!='FIN')
{
	...
	if (table_exists($NombreTabla, $link)) 
	{
		...
		//echo $query_final."<br>";		
		//echo $NombreTabla.'<br>';
		$query = sprintf("(select %s.rfreenet_texto_eventos_%s.evento_texto,clientes_datos.cliente_nombre,%s.instalacion_id,%s.instalacion_nombre,%s.nodo_mac,%s.cliente_id,%s.email_evento,%s.email_fecha_out from %s inner join clientes_datos 
                         ...
                         inner join %s.rfreenet_texto_eventos_%s 
                                    on (%s.rfreenet_texto_eventos_%s.evento_codigo=%s.email_evento AND 
                                        (%s.rfreenet_texto_eventos_%s.evento_tipo = clientes_suscriptores.gw_tipo OR %s.rfreenet_texto_eventos_%s.evento_tipo = -1))
                                        WHERE email_enviado='1' AND %s.gw_id!='0000' ", $db_name_general, $_SESSION['opcion_idioma'], $NombreTabla, $NombreTabla, $NombreTabla, $NombreTabla,$NombreTabla,$NombreTabla,
		...
		if ($fecha_begin != 0)
		{
			$queryaux .= " AND email_fecha_out>='".$fecha_begin."' ";
		}
		if ($fecha_end != 0)
		{
			$queryaux .= " AND email_fecha_out<='".$fecha_end."' ";
		}
...
{
	$query_final .= ") AS tabla_final ORDER BY tabla_final.email_fecha_out DESC";	
	//echo $query_final.'<br>';
...
else
{
	...
	else
	{
		...
		while ($row = mysql_fetch_array($result))
		{
			...
			//echo "<td align=\"center\" width=\"$ancho_3\" class=\"RFNETtextborder\">".$row['email_evento']."</td>";
			if ($row['email_evento'] == 806)
			{
			...
			echo "<td align=\"center\" width=\"$ancho_4\" class=\"RFNETtextborder\">".$row['email_fecha_out']."</td>";
			echo "</tr>";
			$cuenta_filas++;
		}
	}
