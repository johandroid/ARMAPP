<?php
			...
			while($row = mysql_fetch_array($result))
			{
				for ($iInd=1;$iInd<10;$iInd++)
				{
					...
					if (($iValor > 0) && ($iValor != 27) && ($iValor != 28) && ($iValor != 29))
					{
						...
						$OpcionesSalida = $OpcionesSalida."<option value=\"".$iInd."\">".$etiqueta."</option>";
					}
				}
			}
		}
	...
	{
		$query = sprintf("Select nodo_SEN,nodo_TS1,nodo_TS2,nodo_TS3,nodo_TS4,nodo_TS5,nodo_TS6,nodo_NN1,nodo_NN2,nodo_NN3,nodo_NN4,nodo_NN5,nodo_NN6 from cliente_params_nodo where instalacion_id='%s' AND gw_id='%s' AND nodo_ip='%s';", $instalacion, $gw_id, $nodo_ip);
		$result = mysql_query($query,$link);
		if(!$result)
		{
