...
else
{
	...
	{	
		...
		$query = "SELECT instalacion_id,instalacion_nombre,instalacion_zona_horaria FROM cliente_instalaciones";;
		$result = mysql_query($query,$link);
		if(!$result)
		{
			$sResultado="ERROR";
		...
		{		
			while($row = mysql_fetch_array($result))
			{
				...
				$sResultado.="\" zona_horaria=\"";
				$sResultado.=$row['instalacion_zona_horaria'];
				$sResultado.="\" ></instalacion>";
			}
			$sResultado.="</instalaciones>";	
		}
