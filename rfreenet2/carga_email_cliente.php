...
{
	$NombreTabla = "clientes_email_".$cadena_actual;
	$queryaux='';
	if (table_exists($NombreTabla, $link)) 
	{
		...
		$query = sprintf("(select tabla_email_%u.email_id,tabla_texto_eventos.evento_texto,tabla_texto_modbus.evento_texto as evento_texto_modbus, tabla_gw.gw_tipo, tabla_instalaciones.instalacion_nombre,tabla_email_%u.gw_id,tabla_gw.gw_nombre,tabla_email_%u.nodo_mac,tabla_nodos.nodo_nombre,tabla_email_%u.instalacion_id,tabla_email_%u.email_evento,tabla_email_%u.email_fecha_out,tabla_email_%u.email_fecha_in,tabla_email_%u.email_enviado from 
		     %s.%s as tabla_email_%u inner join %s.clientes_datos as tabla_clientes on (tabla_email_%u.cliente_id=tabla_clientes.cliente_id)			 
			 left join %s.rfreenet_texto_modbus_%s as tabla_texto_modbus on (tabla_texto_modbus.evento_codigo=tabla_email_%u.email_evento)
			 inner join %s.cliente_instalaciones as tabla_instalaciones on (tabla_instalaciones.instalacion_id=tabla_email_%u.instalacion_id)
			 inner join %s.cliente_gateways as tabla_gw on (tabla_gw.gw_id=tabla_email_%u.gw_id)
			 left join %s.cliente_nodos as tabla_nodos on (tabla_nodos.nodo_mac=tabla_email_%u.nodo_mac)
			 left join %s.rfreenet_texto_eventos_%s as tabla_texto_eventos on (tabla_texto_eventos.evento_codigo=tabla_email_%u.email_evento AND (tabla_texto_eventos.evento_tipo = tabla_gw.gw_tipo OR tabla_texto_eventos.evento_tipo = -1))
			 left join %s.cliente_analizadores as tabla_analizadores on (tabla_analizadores.analizador_id=tabla_email_%u.nodo_mac AND tabla_texto_eventos.evento_codigo>299 AND tabla_texto_eventos.evento_codigo<400)			 
		WHERE tabla_email_%u.email_evento>100 AND tabla_email_%u.email_evento!='806' AND tabla_email_%u.email_evento!='805' AND tabla_email_%u.email_evento!='800' AND tabla_email_%u.email_evento!='801' AND tabla_email_%u.cliente_id='%s' AND tabla_email_%u.gw_id!='0000' ", 
		...
		$cliente_database, $cuenta_tablas, 
		$db_name_general, $_SESSION['opcion_idioma'], $cuenta_tablas,
		$cliente_database, $cuenta_tablas,$cuenta_tablas,$cuenta_tablas,$cuenta_tablas,$cuenta_tablas,$cuenta_tablas,$cuenta_tablas, $cliente, $cuenta_tablas);
		//echo $query."<br/>";
		if ($instalacion != '0000')
		{
			 $queryaux .= " AND tabla_email_".$cuenta_tablas.".instalacion_id='".$instalacion."' ";
		}
		if ($filtro_gw != '000')
		{
			$queryaux .= " AND tabla_email_".$cuenta_tablas.".gw_id='".$filtro_gw."' ";
		}
		if ($filtro_nodo != '000')
		{
			$queryaux .= " AND tabla_email_".$cuenta_tablas.".nodo_mac='".$filtro_nodo."' ";
		}
		if ($filtro_utc != '000')
		{
			$queryaux .= " AND tabla_email_".$cuenta_tablas.".nodo_mac='".$filtro_utc."' ";
		}
		if ($fecha_begin != 0)
		{
			//$queryaux .= " AND tabla_email_".$cuenta_tablas.".email_fecha_out>='".$fecha_begin."' ";
			$queryaux .= " AND tabla_email_".$cuenta_tablas.".email_fecha_in>='".$fecha_begin."' ";
		}
		if ($fecha_end != 0)
		{
			$queryaux .= " AND tabla_email_".$cuenta_tablas.".email_fecha_out<='".$fecha_end."' ";
		}
		...
		switch ($filtro_evento)
		{
			...
			case 2:
				$queryaux .= " AND (tabla_email_".$cuenta_tablas.".email_evento='800')";
				break;
				
			// Si piden alimentacion, solo bateria y 220
			case 3:
				$queryaux .= " AND ((tabla_email_".$cuenta_tablas.".email_evento='805') OR (tabla_email_".$cuenta_tablas.".email_evento='806') OR (tabla_email_".$cuenta_tablas.".email_evento='801'))";
				break;
				
			// Si piden umbrales
			case 4:
				$queryaux .= "AND (((tabla_email_".$cuenta_tablas.".email_evento>'505') AND (tabla_email_".$cuenta_tablas.".email_evento<'512')) OR ((tabla_email_".$cuenta_tablas.".email_evento>'624') AND (tabla_email_".$cuenta_tablas.".email_evento<'673')) OR ((tabla_email_".$cuenta_tablas.".email_evento>'299') AND (tabla_email_".$cuenta_tablas.".email_evento<'400')))";
				break;
	
			// Si piden gradientes
			case 5:
				$queryaux .= "AND (((tabla_email_".$cuenta_tablas.".email_evento>'674') AND (tabla_email_".$cuenta_tablas.".email_evento<'709')) OR ((tabla_email_".$cuenta_tablas.".email_evento>'511') AND (tabla_email_".$cuenta_tablas.".email_evento<'518')) OR ((tabla_email_".$cuenta_tablas.".email_evento>'299') AND (tabla_email_".$cuenta_tablas.".email_evento<'400')))";
				break;
				
			// Si piden datos, mostraremos solo eventos de datos
			case 1:
				$queryaux .= "AND (((tabla_email_".$cuenta_tablas.".email_evento>'499') AND (tabla_email_".$cuenta_tablas.".email_evento<'506')) OR ((tabla_email_".$cuenta_tablas.".email_evento>'599') AND (tabla_email_".$cuenta_tablas.".email_evento<'609')) OR ((tabla_email_".$cuenta_tablas.".email_evento>'299') AND (tabla_email_".$cuenta_tablas.".email_evento<'400')))";
				break;
...
{
	$query_final .= ") AS tabla_final ORDER BY tabla_final.email_fecha_in DESC";	
...
{
	$numero_email = mysql_num_rows($result);
	echo "<table border=\"1\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\" id=\"tabula_datos\">";
	echo "<tr>";
	echo "<td align=\"center\" width=\"$ancho_1\" class=\"RFNETtextborderBold\">Id</td>";
	echo "<td align=\"center\" width=\"$ancho_2\" class=\"RFNETtextborderBold\">".$idiomas[$_SESSION['opcion_idioma']]['general20']."</td>";
	...
	else
	{
		...
		while ($row = mysql_fetch_array($result))
		{
			...
			$redondeado=sprintf("%04u",$row['email_id']);
			...
			//echo "<td align=\"center\" width=\"$ancho_3\" class=\"RFNETtextborder\">".$row['nodo_mac']."</td>";
			if (strlen($row['nodo_mac']) == 2)
			{
				...
				//echo "<td align=\"center\" width=\"$ancho_4\" class=\"RFNETtextborder\">".$row['email_evento']."</td>";
				echo "<td align=\"center\" width=\"$ancho_4\" class=\"RFNETtextborder\">".utf8_encode($row['evento_texto_modbus'])."</td>";
			}
			else
			{
				if (($row['nodo_nombre'] == "") || ($row['nodo_nombre'] == "NULL"))
			...
			//echo "<td align=\"center\" width=\"$ancho_6\" class=\"RFNETtextborder\">".$row['email_fecha_out']."</td>";
			echo "<td align=\"center\" width=\"$ancho_6\" class=\"RFNETtextborder\">"./*$row['email_fecha_in']*/sObtener_Fecha_Desde_String($cliente_database, $instalacion, $row['email_fecha_in'],$zona_horaria)."</td>";
			echo "</tr>";
			$cuenta_filas++;
		}
	}
